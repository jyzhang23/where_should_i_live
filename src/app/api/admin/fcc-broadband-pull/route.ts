...
import { pullFCCBroadbandData } from "@/lib/admin/pulls";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.info("Starting FCC broadband pull");
    const result = await pullFCCBroadbandData(dataDir, (msg) => logger.info(msg));
    ...
    return NextResponse.json({ success: true, message: "FCC broadband data pull complete", stats: result.stats });
  } catch (error) {
    logger.error("FCC broadband pull failed", { error: error instanceof Error ? error.message : String(error) });
    return NextResponse.json({ error: "Failed to pull FCC broadband data", details: error instanceof Error ? error.message : "Unknown error" }, { status: 500 });
  }
}
